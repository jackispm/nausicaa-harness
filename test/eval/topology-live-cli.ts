...
const maxOutputTokens = Number(process.env.NAUSICAA_TOPOLOGY_OUTPUT_TOKENS ?? "2048");
...
try {
  for (const id of selected as CaseId[]) {
    ...
    try {
      ...
      else if (id === "team-sequential") await teamSequentialCase(f);
...
function policy(tetoEnabled = false) {
  return { maxMainStepsPerActivation: 24, maxModelTokens: 1_000_000, mainRequestTimeoutMs: 125_000,
...
async function oneShot(f: Fixture, prompt: string, tetoEnabled = false): Promise<void> {
  ...
  const result = await executeRun({ workspace: f.workspace, dataDir: f.dataDir, model: model!,
    ...
    policy: policy(tetoEnabled), maxOutputTokens, allowWrite: true,
...
async function openSession(f: Fixture, options: { teto?: boolean; worker?: boolean; resume?: string } = {}) {
  const session = await SessionController.open({ workspace: f.workspace, dataDir: f.dataDir, model: model!,
    policy: policy(options.teto), maxOutputTokens,
...
async function turn(session: SessionController, f: Fixture, prompt: string, timeoutMs = 120_000): Promise<void> {
  f.prompts.push(prompt);
  const inputId = randomUUID();
  await session.submit({ inputId, text: prompt });
  let timedOut = false;
  const timeout = setTimeout(() => { timedOut = true; void session.cancel("Live turn wall-clock limit"); }, timeoutMs);
  try {
    await session.waitForIdle();
    if (timedOut) throw new Error(`Live turn exceeded the test's ${timeoutMs / 1000}s wall-clock limit`);
...
async function teamSequentialCase(f: Fixture) {
  ...
  await oneShot(f, `Please solve the checkout task collaboratively. First create ${JSON.stringify(team)}. After items and policy report, add member total to this same Team with team_assign. Give total their findings as input and ask it to compute subtotal + shipping - discount without reading files. Use member evidence for your final Chinese answer and record acceptance if supported. You remain the coordinator; the members do the calculation.`);
  ...
  f.checks.threeSuccessfulMembers = ["items", "policy", "total"].every((id) => messages(f).some((message) =>
    message.from === `team:checkout:${id}` && message.payload.type === "task.result" && message.payload.status === "completed"));
  ...
  f.checks.assignmentOrder = dependent !== undefined && prerequisites.length === 2 && prerequisites.every((event) => event.globalOffset < dependent.globalOffset);
  const dependentCall = live.calls.find((call) => call.runId === f.runId && call.laneId === "team:checkout:total");
  f.checks.handoffContext = dependentCall !== undefined && /44/.test(dependentCall.context) && /6/.test(dependentCall.context) && /4/.test(dependentCall.context);
...
async function teamCalendarCase(f: Fixture) {
  ...
  try {
    await turn(session, f, `你好，帮我创建一个团队，做一个周日历 + Todo 的 HTML 网站。Create ${JSON.stringify(team)}. You coordinate; members write the files. After both builders report, add qa to this same Team with team_assign. Ask qa to read index.html and app.js, review their integration and required functionality, and include CALENDAR-REVIEWED with inspected file names in its final report. Ask the responsible builder to repair material defects, then ask qa to review again. After the results support acceptance, record it and report the file paths.`, teamTurnTimeoutMs);
    ...
    f.checks.membersSucceeded = ["ui", "logic", "qa"].every((id) => messages(f).some((message) =>
      message.from === `team:calendar:${id}` && message.payload.type === "task.result" && message.payload.status === "completed"));
    ...
    f.checks.membersWriteFiles = ["ui", "logic"].every((member) =>
      ["write_file", "apply_patch", "edit", "edit_file"].some((name) => toolSucceeded(f, `team:calendar:${member}`, name)));
    ...
    f.checks.noForcedPause = !f.events.some((event) => event.laneId === "main"
      && ["turn.waiting", "turn.failed", "turn.cancelled"].includes(event.type));
    ...
    const accepted = f.events.findLast((event) => event.type === "team.presented"
      && event.payload.teamId === "calendar" && event.payload.disposition === "accepted");
    f.checks.finalAnswerAfterAcceptance = accepted !== undefined && f.events.some((event) => event.laneId === "main"
      && event.type === "turn.completed" && event.globalOffset > accepted.globalOffset);
...
async function naturalTeamCalendarCase(f: Fixture) {
  const session = await openSession(f);
  try {
    await turn(session, f, "你好，帮我创建一个团队，做一个极简的周日历 + Todo HTML 网站，放在当前目录。开发和审查由不同成员负责，发现问题就交给原开发成员修复并复验，最后告诉我文件在哪。", teamTurnTimeoutMs);
    await waitUntil(() => f.events.some((event) => event.type === "team.presented")
      || f.events.some((event) => event.laneId === "main" && ["turn.failed", "turn.waiting", "turn.cancelled"].includes(event.type)), 240_000);
    await session.waitForIdle();
    const created = f.events.filter((event) => event.type === "team.created");
    f.checks.oneTeam = created.length === 1;
    const results = messages(f).filter((message) => message.payload.type === "task.result" && message.from.startsWith("team:"));
    const memberLanes = new Set(results.map((message) => message.from));
    f.checks.multipleMembers = memberLanes.size >= 2;
    f.checks.membersUsedTools = [...memberLanes].every((lane) => f.events.some((event) => event.laneId === lane && event.type === "tool.succeeded"));
    f.checks.memberWroteArtifact = [...memberLanes].some((lane) => ["write_file", "apply_patch", "edit", "edit_file"].some((name) => toolSucceeded(f, lane, name)));
    f.checks.reportsInGroup = results.length > 0 && results.every((message) => {
      if (message.payload.type !== "task.result") return false;
      const threadId = `task:${message.payload.taskId}`;
      return f.events.some((event) => event.type === "team.message.sent" && event.payload.fromLane === message.from
        && event.payload.threadId === threadId);
    });
    f.checks.resultsConsumed = results.length > 0 && results.every((message) => consumed(f, "main", message.messageId));
    f.checks.noForcedPause = !f.events.some((event) => event.laneId === "main" && ["turn.waiting", "turn.failed", "turn.cancelled"].includes(event.type));
    const presented = f.events.findLast((event) => event.type === "team.presented" && event.payload.disposition === "accepted");
    f.checks.accepted = presented !== undefined;
    f.checks.finalAnswerAfterAcceptance = presented !== undefined && f.events.some((event) => event.laneId === "main"
      && event.type === "turn.completed" && event.globalOffset > presented.globalOffset);
    const entries = await readdir(f.workspace);
    const htmlNames = entries.filter((name) => name.endsWith(".html"));
    f.checks.htmlArtifact = htmlNames.length > 0;
    const html = (await Promise.all(htmlNames.map((name) => readFile(join(f.workspace, name), "utf8")))).join("\n");
    const scripts = [...html.matchAll(/<script\b[^>]*>([\s\S]*?)<\/script>/giu)].map((match) => match[1] ?? "");
    scripts.push(...await Promise.all(entries.filter((name) => name.endsWith(".js")).map((name) => readFile(join(f.workspace, name), "utf8"))));
    for (const script of scripts) new Script(script);
    f.checks.javascriptParses = scripts.some((script) => script.trim().length > 0);
    const transcript = await session.transcript();
    f.finalText = transcript.findLast((entry) => entry.role === "assistant")?.content ?? "";
    f.checks.reportsActualPath = htmlNames.some((name) => f.finalText.includes(name));
  } finally { await session.close(); }
}
