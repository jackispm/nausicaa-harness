...
Nausicaa 是一个处于设计阶段的高性能 Agent harness。它面向长程任务，核心不是再造一个模型 SDK，而是探索一种更有效的运行方式：一条主线负责推进工作，多条低成本辅助线持续观察、质疑、发散和纠偏。
...
- Ledger 记录事实，RLM runtime 提供按需查询，Agent 负责判断。

这一组机制暂称为 **Live Observer Lane + Ledger-native Soft Advice Protocol**。它是待验证的核心创新假设，不宣称 Dream/Reflection 或并行 Agent 本身是原创。
...
底层优先使用 [`pi-ai`](https://github.com/earendil-works/pi/tree/main/packages/ai) 处理 provider、模型和流式调用。Nausicaa 自己拥有 Ledger、lane 调度、RLM 查询接口、A2A inbox、插件边界和运行策略；是否复用 `pi-agent-core` 必须由实验决定，而不是默认继承其 loop 语义。
...
- [`docs/architecture.md`](docs/architecture.md)：运行时分层、Ledger、RLM、lane 和 A2A 设计。
